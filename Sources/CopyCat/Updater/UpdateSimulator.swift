...
/// Debug-only fake updater that walks the update UI through scripted update
/// scenarios with realistic pacing — no Sparkle, no signing, no appcast.
///
/// Enable, then launch with `just dev`:
///
///     defaults write com.copycat.macos.app.dev UpdateSimulatorScenario happy
///
/// Disable:
///
///     defaults delete com.copycat.macos.app.dev UpdateSimulatorScenario
@MainActor
final class UpdateSimulator: UpdaterProviding {
    ...
    private func offerUpdate(notify: Bool) {
        ...
        if notify {
            ...
            content.body = "CopyCat 99.0 is available. Click to update."
