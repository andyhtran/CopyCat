...
/// that the menu and Settings UI render inline, replacing Sparkle's own alert
/// and progress windows entirely.
@MainActor
final class UpdateDriver: NSObject, SPUUserDriver {
    ...
    func showUpdateFound(
        ...
        // discovery with a notification; tapping it opens Settings where the
        // update controls live. For user-initiated checks the banner is
        // already visible — just drop any stale one.
    ...
    func showUpdateReleaseNotes(with downloadData: SPUDownloadData) {
        // Release notes aren't rendered in the inline update UI.
    }
    ...
    func showUpdateNotFoundWithError(
        ...
        // acknowledged, and the menu may never be opened — so acknowledge on
        // a timer, which also auto-dismisses the "up to date" banner.
    ...
    private func postUpdateAvailableNotification(version: String) {
        ...
        content.body = "CopyCat \(version) is available. Click to update."
        ...
        // the menu and Settings surfaces still cover discovery.
